...
`Plugins-SDK` 是 `Qingci-Bot` 根目录下的**多个独立子项目之一**，与 `Qingci-Bot-CE`、`Plugin-Market` 平级。它的定位是：
...
- **插件的开发与验证环境**：`plugins/` 下内置 `_template`（完整模板），开发者复制模板即可开始写插件；最小示例见独立仓库 [Qingci-Bot/hello](https://github.com/Qingci-Bot/hello)。
